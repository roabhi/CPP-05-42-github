...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/01 18:37:50 by rabril-h          #+#    #+#             */
/*   Updated: 2023/11/01 18:48:56 by rabril-h         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	doCheckFormGradeTest( unsigned int& testNumber, AForm* form, unsigned int signExpectedValue, unsigned int execExpectedValue )
{
	...
	std::cout << C_BLUE << "Test " << testNumber++ << ": " << C_PINK << "Check exec grade in " << form->getName() << " class: " \
				<< form->getName() << "\tform" << C_WHITE << std::endl;
	std::cout << "\tExpected value: " << execExpectedValue << " | Resultant value: " << form->getGradeToExe() << std::endl;
...
void	testsBureaucratExecuteForm( void )
{
	...
	Bureaucrat		JuanMin( "Mini Juan", GRADE_MIN );
	Bureaucrat		JuanMid( "Medium Juan", GRADE_AVG );
	Bureaucrat		JuanMax( "Maximun Juan", GRADE_MAX );
	int				invalidFile;
	unsigned int	testNumber;
